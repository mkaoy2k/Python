...
使用多執行緒並行處理大量圖像，支援高斯模糊和縮放處理。
改動 process_images_CP.py：
- 將 ProcessPoolExecutor 改為 ThreadPoolExecutor
- 更新了相關的文件字串，將「多進程」改為「多執行緒」

這個改動會讓程式使用執行緒池(Thread Pool)而不是進程池(Process Pool)來處理圖像。
由於圖像處理主要涉及檔案讀取和寫入等 I/O 操作，使用執行緒池通常會比進程池更有效率，
因為：
- 執行緒的創建和切換開銷比進程小
- 執行緒可以更好地利用 I/O 等待時間
- 執行緒之間的通訊比進程之間更簡單
