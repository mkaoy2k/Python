...
from biTree import BinaryTreeNode
...
# 3. 印出鍵值
print(f'範例 2.1: 以中序方式印出鍵值： {node_root.print_keys("Inorder")}')
print()
print(f'範例 2.2: 以後序方式印出鍵值： {node_root.print_keys("Postorder")}')
print()
print(f'範例 2.3: 以先序方式印出鍵值： {node_root.print_keys("Preorder")}')
print()

# 4. 印出排序後的鍵值列表
print(
    f'範例 2.4: 排序整個二元樹的鍵值： {BinaryTreeNode.root.sort_keys()}，共 {BinaryTreeNode.count} 個節點')
...
if node_found != -2:
    node_max = node_found.find_node_max()
    print(
        f'範例 3.3: 鍵值 = {key} 的子樹中，局部最大鍵值 = {node_max.key} 位於 {node_max}')
    node_min = node_found.find_node_min()
    print(
